...
ivreg2 cov_male (z_eduyears2 z_out_intell=allele_score_ea allele_score_sniekers) cov_comp_bodysize10 cov_comp_height10 cov_birthweight cov_dist_lon cov_birth_location_imd cov_birth_location_easting cov_birth_location_northing ///
	cov_breastfed cov_father_alive cov_mother_alive cov_num_sisters cov_num_brothers cov_matsmoking imob_* yob_* pc_* cov_male if (within_fam_id ==1|within_fam_id ==.),ro first cluster(mobi) endog(z_eduyears2 z_out_intell) partial(cov_male imob_* yob_* pc_*)
matrix X=e(first)
local ea_f=X[8,1]
local intell_f=X[8,2]

di "`ea_f'"
regsave using "results/ivreg2_outcome_adjusted", detail(all) pval ci replace  addlabel(outcome,"`i'",intell_f,`intell_f',ea_f,`ea_f') 
...
foreach j in  `r(varlist)'{
	...
	ivreg2 `j' (z_eduyears2 z_out_intell=allele_score_ea allele_score_sniekers) cov_comp_bodysize10 cov_comp_height10 cov_birthweight cov_dist_lon cov_birth_location_imd cov_birth_location_easting cov_birth_location_northing ///
	cov_breastfed cov_father_alive cov_mother_alive cov_num_sisters cov_num_brothers cov_matsmoking imob_* yob_* pc_* cov_male  if  (within_fam_id ==1|within_fam_id ==.),ro first cluster(mobi) endog(z_eduyears2 z_out_intell)  partial(cov_male imob_* yob_* pc_*)
	matrix X=e(first)
	local ea_f=X[8,1]
	local intell_f=X[8,2]
	regsave z_eduyears2 z_out_intell using "results/ivreg2_outcome_adjusted", detail(all) pval ci append  addlabel(outcome,"`j'",intell_f,`intell_f',ea_f,`ea_f') 
	}
...
foreach j in  `r(varlist)'{
	di "`j'"
	ivreg2 `j' (z_eduyears2 z_out_intell=allele_score_ea allele_score_sniekers) cov_comp_bodysize10 cov_comp_height10 cov_birthweight cov_dist_lon cov_birth_location_imd cov_birth_location_easting cov_birth_location_northing ///
	cov_breastfed cov_father_alive cov_mother_alive cov_num_sisters cov_num_brothers cov_matsmoking imob_* yob_* pc_* cov_male  if (within_fam_id ==1|within_fam_id ==.),ro first endog(z_eduyears2 z_out_intell) partial(cov_male imob_* yob_* pc_*) cluster(mobi) 
	matrix X=e(first)
	local ea_f=X[8,1]
	local intell_f=X[8,2]
	regsave z_eduyears2 z_out_intell  using "results/ivreg2_outcome_adjusted", detail(all) pval ci append addlabel(outcome,"`j'",intell_f,`intell_f',ea_f,`ea_f') 	
...
use "results/ivreg2_outcome_adjusted",clear
...
//Clean bivariate results
	...
	graph export "results/IPD_figure_3_bivariate_ivreg_bin_adj.pdf", as(pdf) replace fontface("Calibri (Body)");
	graph save "results/IPD_figure_3_bivariate_ivreg_bin_adj", replace;
	#delimit cr
	...
	graph save "results/IPD_figure_3_bivariate_ivreg_cont_adj", replace 

	graph combine "results/IPD_figure_3_bivariate_ivreg_bin_adj" "results/IPD_figure_3_bivariate_ivreg_cont", col(1) imargin(0 0 0 0) ysize(8) xsize(6.26) graphregion(color(white))
	graph export "results/IPD_figure_3_bivariate_`x'_ivreg._adj.eps", as(pdf) replace fontface("Calibri")
