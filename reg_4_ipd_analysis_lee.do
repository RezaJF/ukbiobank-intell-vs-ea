...
//It uses the Okbay discovery sample and the cognition score at p<5E-06
...
ivreg2 cov_male (z_eduyears2 z_out_intell=allele_score_ea3 allele_score_sniekers)  imob_* yob_* pc_* cov_male if _n<_N*.01,ro first cluster(mobi) endog(z_eduyears2 z_out_intell) partial(cov_male imob_* yob_* pc_*)
matrix X=e(first)
local ea_f=X[8,1]
local intell_f=X[8,2]

di "`ea_f'"
regsave using "results/ivreg2_outcome_lee", detail(all) pval ci replace  addlabel(outcome,"`i'",intell_f,`intell_f',ea_f,`ea_f') 
...
foreach j in  `r(varlist)'{
	...
	ivreg2 `j' (z_eduyears2 z_out_intell=allele_score_ea3 allele_score_sniekers)  imob_* yob_* pc_* cov_male,ro first cluster(mobi) endog(z_eduyears2 z_out_intell)  partial(cov_male imob_* yob_* pc_*)
	matrix X=e(first)
	local ea_f=X[8,1]
	local intell_f=X[8,2]
	regsave z_eduyears2 z_out_intell using "results/ivreg2_outcome_lee", detail(all) pval ci append  addlabel(outcome,"`j'",intell_f,`intell_f',ea_f,`ea_f') 
	}
...
foreach j in  `r(varlist)'{
	di "`j'"
	ivreg2 `j' (z_eduyears2 z_out_intell=allele_score_ea3 allele_score_sniekers)  imob_* yob_* pc_* cov_male,ro first endog(z_eduyears2 z_out_intell) partial(cov_male imob_* yob_* pc_*) cluster(mobi) 
	matrix X=e(first)
	local ea_f=X[8,1]
	local intell_f=X[8,2]
	regsave z_eduyears2 z_out_intell  using "results/ivreg2_outcome_lee", detail(all) pval ci append addlabel(outcome,"`j'",intell_f,`intell_f',ea_f,`ea_f') 	
...
use "results/ivreg2_outcome_lee",clear
...
//Clean bivariate results
	...
	graph export "results/IPD_lee_figure_3_bivariate_`x'_ivreg.eps", as(pdf) replace fontface("Calibri")
